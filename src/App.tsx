      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col justify-between">
        {activeView === 'cards' && (
          <div className="space-y-6 flex-1">
            ...
            {/* Overnight Shift Notification Banner (Turno a cavallo di mezzanotte in corso da ieri sera) */}
            {(() => {
              const yesterdayDate = getYesterdayDateString(today);
              const pendingOvernight = employees
                .map((emp) => {
                  const yestRec = records.find(
                    (r) => r.employeeId === emp.id && r.date === yesterdayDate
                  );
                  if (yestRec && yestRec.clockInAfternoon && !yestRec.clockOutAfternoon) {
                    return { emp, rec: yestRec };
                  }
                  return null;
                })
                .filter(Boolean) as { emp: Employee; rec: TimeRecord }[];

              if (pendingOvernight.length === 0) return null;

              return (
                <div className="bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-900 text-white rounded-2xl p-4 sm:p-5 shadow-sm border border-indigo-700/50 space-y-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-white tracking-wide">
                        Turno notturno a cavallo della mezzanotte in corso
                      </h3>
                      <p className="text-xs text-indigo-200">
                        Dipendenti con turno pomeridiano iniziato ieri ({formatDateIT(yesterdayDate)}) e non ancora chiuso:
                      </p>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                    {pendingOvernight.map(({ emp, rec }) => (
                      <div
                        key={emp.id}
                        className="bg-white/10 backdrop-blur-xs border border-white/15 rounded-xl p-3 flex items-center justify-between gap-3"
                      >
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-white truncate">{emp.name}</p>
                          <p className="text-[11px] text-indigo-200 font-mono">
                            Entrata ieri: {rec.clockInAfternoon}
                          </p>
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                            ...
                            const updatedRec: TimeRecord = {
                              ...rec,
                              clockOutAfternoon: now,
                              clockOutAfternoonNextDay: true,
                              updatedAt: new Date().toISOString(),
                            };
                            handleSaveRecord(updatedRec);
                            showToast(`Uscita registrata per ${emp.name} alle ${now} (+1 gg 🌙)`);
                          }}
                          className="px-3 py-1.5 bg-indigo-500 hover:bg-indigo-400 text-white rounded-lg text-xs font-bold transition-all shadow-xs shrink-0 cursor-pointer flex items-center gap-1.5"
                        >
                          <Moon className="w-3.5 h-3.5" />
                          <span>Timbra Uscita ({systemTimeHHMM})</span>
                        </button>
                      </div>
                    ))}
                  </div>
